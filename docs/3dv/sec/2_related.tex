...
prompt; SG-Nav~\cite{yin2024sgnav} adds re-perception to that serialization precisely
because wrong node identity poisons downstream use;
...
Beacon3D~\cite{huang2025beacon3d} decouples grounding from answering and chains them
into per-object coherence cases, at far larger scale than ours;
MV-ScanQA~\cite{mo2025mvscanqa} formalises cross-view necessity; and Jin et
al.~\cite{jin2025revisiting3d} find 2D VLMs on rendered views match 3D LLMs.
Fine-grained 3D-VL diagnosis is therefore not the contribution here. What these
stage-wise analyses do is \emph{observe}: they
compare the quality of stored representations~\cite{gu2024conceptgraphs}, correlate
inference-time stages over an already-annotated scene~\cite{huang2025beacon3d}, or
report an oracle bound from a \emph{different
system}~\cite{azuma2022scanqa, takmaz2023openmask3d}. StageReach3D \emph{intervenes}:
scope-aware oracle substitutions on declared stage paths, with executable consistency
checks, place an oracle-fed and a pipeline-labelled number side by side on the same
questions over the same store, under an evaluator that enforces the distinction.
GraphEQA illustrates the stake: its benchmark numbers use ground-truth segmentation
masks and its detector-fed evaluation is a few unaggregated real-world trials, so its
held-versus-delivered gap is not recoverable.
...
abstention-worthy uncertainty originates, nor expresses a system that should have
answered because its own store held the fact, and did not reach it.
