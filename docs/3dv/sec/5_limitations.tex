...
count, and we report the disagreement rather than settle it. An independent human annotator,
blinded to system outputs, arm names and the hypothesis, answered all 12 items from
the committed frames: \SRAnnScoredAgree{}/\SRAnnScoredOf{} scored answers agree;
\SRAnnAbstainDisagreements{} of the 4 disagreements are confidence-1 abstentions, the
fourth a confidence-3 set over-inclusion, and neither excluded item was marked
ambiguous \rid{G12}.
...
with three clusters (\cref{sec:component}). The fault battery's injectors and checkers
share the evaluator's ontology, so its \SRFaultLocalized{} is internal-consistency
validation only. Scope enforcement itself is schema-level: it verifies declared paths
and statuses under trusted, correctly-declared adapters, and cannot detect a
misdeclared adapter leaking oracle information (\cref{sec:evaluator}).
